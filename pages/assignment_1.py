...
vector = np.load("/workspaces/word2vec-qa-chatbot-annaji1/data/advanced-vector.npz")
ques_vec = vector['x']
ans_vec = vector['y']

# load the trained word2vec model 
# Hint: You should use the word2vec model pre-trained with both question and answer sets.
trained_w2v = gensim.models.Word2Vec.load("/workspaces/word2vec-qa-chatbot-annaji1/data/w2v-advanced.model")
...
def trained_sentence_vec(sent):
    ...
    qu_voc = [i for i in sent if i in trained_w2v.wv]
    # Get the embedding of the characters
    # Hint: Stack arrays in sequence vertically using np.vstack
    emb = np.vstack([trained_w2v.wv[i] for i in qu_voc])
...
def find_answer(qr_sentence, ques_vec, ans_vec):
    ...
    if ans_idx >= n_q_a:
        sim = cosine_similarity(q, ans_vec)
        ans_idx = np.argmax(sim)              
    return ans_idx
